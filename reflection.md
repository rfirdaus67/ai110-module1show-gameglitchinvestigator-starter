...
I used both Claude and ChatGPT for help. One issue I had to fix was the "Attempts Left" section not decrementing after the first guess. AI suggested i use st.rerun to reload the page after the guess and I verified this by running the program and seeing if the display decremented after submitting my first guess. Ai specifically said, "Fix option 1 (best): force a rerun after increment. Add st.rerun() after processing the guess so the UI refreshes immediately." An AI suggestion that was misleading was after the st.rerun line, my hints were no longer showng up. It seemed there wasn't a simple fix to get both working and I needed to change multiple lines of code. ChatGPT suggested I restructure the submit block and make a new session state variable for the hint, so I didn't have to use st.rerun, but this was incorrect. I checked this because I would either get an error, the hints were not showing up or the "Attempts Left" wasn't decrementing after the first guess. It turned out I needed to use a combination of these things to get my program working.
